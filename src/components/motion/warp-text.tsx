...
/**
 * React Bits "WarpText" (JS + CSS variant), ported to TypeScript as-is: a canvas-rasterised text
 * texture distorted by a WebGL glass/refraction shader, with idle drift and pointer lensing.
 * Logic is unchanged from the source — only typed and stripped of the standalone CSS file (this
 * project has no CSS-module convention; the two rules it needed are inlined as Tailwind classes on
 * the wrapper and set directly on the canvas via JS, exactly as the original component already did).
 */
...
export type WarpTextProps = {
  ...
  refraction?: number;
  ripple?: boolean;
  /** Fraction of the box width/height the rasterised text is allowed to fill before the auto-fit
   *  shrinks it. The upstream defaults (0.86 / 0.78) assume a roomy hero-style box with margin
   *  around the text; a box sized tightly to the text's own bounding rect (see `WarpHeading`) needs
   *  these close to 1, or the auto-fit shrinks text that already fits, with nothing gained. */
  fitWidth?: number;
  fitHeight?: number;
  /** Set false to skip the auto-fit shrink entirely and always draw at the computed font size. The
   *  fit's own width measurement sums per-character `ctx.measureText()` calls plus a manually
   *  re-added letter-spacing, which — versus the browser's real shaped/kerned text run — reliably
   *  overestimates width by a few percent. That's an intentional safety margin for a roomy, unmeasured
...
const buildTextCanvas = ({
  ...
  const fit = props.fitText ? Math.min(1, maxWidth / widest, maxHeight / blockHeight) : 1;
...
const syncUniforms = (program: Program, props: RasterProps) => {
...
/**
 * Renders only the canvas; the caller is responsible for its own layout box (see `HeaderWarp`,
 * which overlays this absolutely on top of a normal, always-in-flow heading).
 */
